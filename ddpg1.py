...
def getStep(inistate, action):
    ...
    r_extra = (state[18]-state[28]) * 4
    r_punish1 = - min(abs(state[8]-state[18]), abs(state[8]-state[28])) * 4
    r_move = (state[8]-inistate[8]) + (state[18] - inistate[18]) + (state[28]-inistate[28]) + (state[38]-inistate[38]) + (state[48]-inistate[48])
    r_move = r_move * 12

    r_punish2 = -np.exp(-(state[20] - state[30]) ** 2)



    reward = r_torso + r_Rthigh + r_Lthigh + r_Rcalf + r_Lcalf + r_torso_anglex + r_Rthigh_anglex + r_Lthigh_anglex + \
             r_Rcalf_anglex + r_Lcalf_anglex + r_torso_anglez + r_Rthigh_anglez + r_Lthigh_anglez + r_Rcalf_anglez + \
             r_Lcalf_anglez + 5 * (r_pos_z + r_pos_z_torso + r_pos_z_Rthigh + r_pos_z_Lthigh + r_pos_z_Rcalf +
                                 r_pos_z_Lcalf) + 5 * r_pos_x + r_extra + r_punish1 + r_move + r_punish2
    # reward = 0
...
def playGame(train_indicator=0):    #1 means Train, 0 means simply Run
    ...
    try:
        actor.model.load_weights("model1/actormodel.h5")
        critic.model.load_weights("model1/criticmodel.h5")
        actor.target_model.load_weights("model1/actormodel.h5")
        critic.target_model.load_weights("model1/criticmodel.h5")
        ismodel = True
    ...
    for i in range(episode_count):
        ...
        for j in range(max_steps):
            ...
            if (i == 0) and (ismodel == False):
                a_t = np.array([-1.2, -0.05, 0, 0, 0, 0, 0.5, 0.05, 0.3, 0.03, 0, 0])
            else:
                if j % 4 == 0:
                    #a_t = np.array([-0.5, -0.05, 0, 0, 0, 0, 0.5, 0.05, 0.3, 0.03, 0, 0])
                    ...
                    #a_t = np.array([0.2, 0.02, 0, 0, 0, 0, -0.2, -0.02, 0, 0, 0, 0])
                    a_t = actor.model.predict(s_t.reshape(1, s_t.shape[0])).flatten()
                    a_t[0] = abs(a_t[0])
                    a_t[1] = abs(a_t[1])
                    a_t[2] = min(0, a_t[2])
                    ...
                    #a_t = np.array([0, 0, -0.5, -0.05, 0.5, 0.05, 0, 0, 0, 0, 0.3, 0.03])
                    a_t = actor.model.predict(s_t.reshape(1, s_t.shape[0])).flatten()
                    a_t[0] = min(0, a_t[0])
                    a_t[1] = min(0, a_t[1])
                    a_t[2] = -abs(a_t[2])
                    ...
                    #a_t = np.array([0, 0, 0.5, 0.05, -0.5, -0.05, 0, 0, 0, 0, 0, 0])
                    a_t = actor.model.predict(s_t.reshape(1, s_t.shape[0])).flatten()
                    a_t[0] = min(0, a_t[0])
                    a_t[1] = min(0, a_t[1])
                    a_t[2] = abs(a_t[2])
                ...
                #for i in range(8, 12):
                    #a_t[i] = 0
                #for i in range(0, len(a_t), 2):
                    #if a_t[i] < 0.2 : a_t[i] = 0.2

                    #elif a_t[i] < -1.5 : a_t[i] = -1.5
            ...
            if (train_indicator):
                ...
                p_max = np.max(compute_a)
                p_min = np.min(compute_a)
                print("grads:",grads[grads.shape[0]-1])
                for k in range(action_dim):
                    if grads[grads.shape[0]-1][k] >= 0:
                        grads[grads.shape[0]-1][k] *= ((0.9 - compute_a[k])/0.9)
                    else:
                        grads[grads.shape[0]-1][k] *= (compute_a[k]/0.9)
        ...
        if np.mod(i, 3) == 0:
            if (train_indicator):
                ...
                actor.model.save_weights("model1/actormodel.h5", overwrite=True)
                with open("actormodel.json", "w") as outfile:
                    json.dump(actor.model.to_json(), outfile)

                critic.model.save_weights("model1/criticmodel.h5", overwrite=True)
